...
//解析后的配置
exports.config = {
	database_default: 'default', //默认数据库
	database: {
		master:{}, //主库
		slave:{}   //从库
	},
 ...
 */
exports.load = function(src, callback){
	if(src===undefined){
	...
	if(isPath(src)){
		...
		fs.readFile(src, {encoding: 'utf8'}, function (err, data) {
			if(err){
				return callback(util.errWrap("The config file is not found ! \n"+err));
			}
	...
	// 检测成功 正式开始解析
	configPretreatment(src, callback);
...
function configPretreatment(text, callback){
	...
	if(parseInt(result)>=0){
		...
		return callback('[TribeDB Error]: Error configuration on line '+err.num+' : "'
			+err.row+'"');
	}
...
function configParse(conf){
	...
	if(!config.database['default']){
		//第一个定义的数据库被当成默认的数据库
		config.database_default = first_db;
	}
