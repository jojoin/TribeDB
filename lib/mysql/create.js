...
Query.prototype.or_where_between= function(name, value, value2){
	if(value2){
		value = [value, value2];
	}
	return this.where(name, value, 'BETWEEN', 'OR');
};

Query.prototype.or_where_not_between= function(name, value, value2){
	if(value2){
		value = [value, value2];
	}
	return this.where(name, value, 'NOT BETWEEN', 'OR');
};
...
Query.prototype.build_join = function(type){
	...
	function deal(one){
		...
		if(type=='insert'){ // join(table, data, key)
			...
			data.key = one[2] || 'id'; // 默认为 “id”
			if(data[3]===null)
				data.key = null; // 不指定联合字段
			return data;
...
Query.prototype.count  =  function(callback, opt){
	var field = 'count(*)';
	this.columns(field);
	this.select(function(err, data){
		// log(data);
		if(err){
			return callback(err);
		}
