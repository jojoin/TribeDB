...
process.insert = function(datas, obj, query, tcnf, callback){
	...
	*/
	...
	// 联合、拆分表插入
	function join_insert(data){
		var insert = obj._join;
		if(!insert){
		...
		for(var i in insert){
			...
			var si = insert[i]
			  ...
			  , jdata = si.data;
			//拆分表主键赋值
			if(key && jdata[key]===undefined){
				if(data[key]!==undefined){
					jdata[key] = data[key]; //指定联合字段值
				}else if(){
					jdata[key] = insertId;
				}
			}
			var db = create.Create(table).data(jdata)
			db.insert(function(err, dat){
